  ...
  if ns_feature_name:
    ...
    if note_offset_feature_name or note_confidence_feature_name:
      offset_times = (
          ...
          else onset_times + noteSequences.Consts.DEFAULT_NOTE_DURATION
      )
      ...
      velocities = np.ceil(
          ...
          else noteSequences.DEFAULT_VELOCITY * np.ones_like(onset_times)
      ).astype(int)
      assert len(onset_times) == len(velocities)

      ns = noteSequences.note_arrays_to_note_sequence(
...
def _examples_to_pianorolls(
  ...
  for i, (target, pred) in enumerate(zip(targets, predictions)):
    ...
    if track_specs is not None:
      target_tracks = [noteSequences.extract_track(target_ns,
                                                    spec.program, spec.is_drum)
                       for spec in track_specs]
      pred_tracks = [noteSequences.extract_track(pred_ns,
                                                  spec.program, spec.is_drum)
                     for spec in track_specs]
    else:
...
def prettymidi_pianoroll(
    ...
    fps: float,
    num_seconds=Consts.DEFAULT_AUDIO_SECONDS
...
def audio_summaries(
    ...
    num_seconds: float = Consts.DEFAULT_AUDIO_SECONDS
) -> Mapping[str, seqio.metrics.MetricValue]:
...
def transcription_summaries(
    ...
    track_specs: Optional[Sequence[noteSequences.TrackSpec]] = None,
    num_seconds: float = Consts.DEFAULT_AUDIO_SECONDS,
    pianoroll_frames_per_second: float = Consts.DEFAULT_PIANOROLL_FRAMES_PER_SECOND,
) -> Mapping[str, seqio.metrics.MetricValue]:
