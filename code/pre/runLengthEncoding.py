...
class EventEncodingSpec:
    ...
    encode_event_fn: Callable[[EncodingState, EventData, eventCodec.Codec],
    ...
    decode_event_fn: Callable[
        [DecodingState, float, eventCodec.Event, eventCodec.Codec], None]
...
def encode_and_index_events(
    ...
    encode_event_fn: Callable[[ES, T, eventCodec.Codec],
                              Sequence[eventCodec.Event]],
    codec: eventCodec.Codec,
    frame_times: Sequence[float],
    encoding_state_to_events_fn: Optional[
        Callable[[ES], Sequence[eventCodec.Event]]] = None,
    ...
    Args:
      ...
      encode_event_fn: Function that transforms event value into a sequence of one
          or more eventCodec.Event objects.
      codec: An eventCodec.Codec object that maps Event objects to indices.
      frame_times: Time for every audio frame.
      encoding_state_to_events_fn: Function that transforms encoding state into a
          sequence of one or more eventCodec.Event objects.
...
def remove_redundant_state_changes_fn(
    ...
    Args:
      codec: The eventCodec.Codec used to interpret the events.
    ...
    def remove_redundant_state_changes(
        ...
        for event in features[feature_key]:
            ...
            if not is_redundant:
                output = np.concatenate([output, [event]])
...
def decode_events(
    ...
    codec: eventCodec.Codec,
    decode_event_fn: Callable[[DS, float, eventCodec.Event, eventCodec.Codec],
                              None],
) -> Tuple[int, int]:
    ...
    Args:
      ...
      codec: An eventCodec.Codec object that maps indices to Event objects.
