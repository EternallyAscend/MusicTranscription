...
def add_unique_id(ds):
    """Add unique integer ID to each example in a dataset."""
    def add_id_field(i, ex):
        ex['unique_id'] = np.array([i])
        return ex
    return np.array([(add_id_field(i, ex)) for i, ex in enumerate(ds)], dtype=object)
...
def pad_notesequence_array(ex):
  ...
  ex['sequence'] = np.pad(np.expand_dims(ex['sequence'], 0),
                          ((0, len(ex['input_times']) - 1), (0,0), (0,0)))
  return ex
...
def tokenize_transcription_example(
  ...
  def tokenize(sequence, audio, sample_rate, example_id=None):
    ...
    noteSequences.validate_note_sequence(ns)
    ...
    if onsets_only:
      times, values = noteSequences.note_sequence_to_onsets(ns)
    else:
      ns = note_seq.apply_sustain_control_changes(ns)
      times, values = (
          noteSequences.note_sequence_to_onsets_and_offsets_and_programs(ns))
    ...
    (events, event_start_indices, event_end_indices,
     state_events, state_event_indices) = (
         runLengthEncoding.encode_and_index_events(
             state=noteSequences.NoteEncodingState() if include_ties else None,
             event_times=times,
             event_values=values,
             encode_event_fn=noteSequences.note_event_data_to_events,
             codec=codec,
             frame_times=frame_times,
             encoding_state_to_events_fn=(
                 noteSequences.note_encoding_state_to_events
...
def tokenize_example_with_program_lookup(
  ...
  def tokenize(sequences, inst_names, audio, example_id=None):
    ...
    noteSequences.assign_instruments(ns)
    noteSequences.validate_note_sequence(ns)
...
def tokenize_slakh_example(
    ...
    onsets_only: bool,
    include_ties: bool,
    track_specs: Optional[Sequence[noteSequences.TrackSpec]],
    ignore_pitch_bends: bool
) -> tf.data.Dataset:
  ...
  def tokenize(sequences, samples, sample_rate, inst_names, example_id):
    ...
    if is_training_data:
      ...
      ns = noteSequences.trim_overlapping_notes(ns)
  ...
  def process_record(input_record):
    ds = tf.data.Dataset.from_generator(
        ...
        args=[
            input_record['noteSequences'], input_record['mix'],
