...
def note_arrays_to_note_sequence(
  ...
  for onset_time, offset_time, pitch, velocity, program, is_drum in itertools.zip_longest(
    ...
    if offset_time is None:
      offset_time = onset_time + Consts.DEFAULT_NOTE_DURATION
    if velocity is None:
      velocity = Consts.DEFAULT_VELOCITY
    if program is None:
      program = 0
    if is_drum is None:
      is_drum = False
...
def note_event_data_to_events(
  ...
  if value.velocity is None:
    ...
    if value.program is None:
      ...
      return [eventCodec.Event('velocity', velocity_bin),
              eventCodec.Event('pitch', value.pitch)]
    else:
      if value.is_drum:
        # drum events use a separate vocabulary
        return [eventCodec.Event('velocity', velocity_bin),
                eventCodec.Event('drum', value.pitch)]
      else:
        # program + velocity + pitch
        if state is not None:
          state.active_pitches[(value.pitch, value.program)] = velocity_bin
        return [eventCodec.Event('program', value.program),
                eventCodec.Event('velocity', velocity_bin),
                eventCodec.Event('pitch', value.pitch)]
...
def note_encoding_state_to_events(
  ...
  for pitch, program in sorted(
    ...
    if state.active_pitches[(pitch, program)]:
      events += [eventCodec.Event('program', program),
                 eventCodec.Event('pitch', pitch)]
  events.append(eventCodec.Event('tie', 0))
  return events
...
class NoteDecodingState:
  ...
  current_velocity: int = Consts.DEFAULT_VELOCITY
...
def decode_note_onset_event(
    ...
    time: float,
    event: eventCodec.Event,
    codec: eventCodec.Codec,
) -> None:
  """Process note onset event and update decoding state."""
  if event.type == 'pitch':
    state.note_sequence.notes.add(
        start_time=time, end_time=time + Consts.DEFAULT_NOTE_DURATION,
        pitch=event.value, velocity=Consts.DEFAULT_VELOCITY)
    state.note_sequence.total_time = max(state.note_sequence.total_time,
                                         time + Consts.DEFAULT_NOTE_DURATION)
  else:
...
def _add_note_to_sequence(
  ...
  end_time = max(end_time, start_time + Consts.MIN_NOTE_DURATION)
  ns.notes.add(
...
def decode_note_event(
  ...
  if event.type == 'pitch':
    ...
    offset_time = time + Consts.DEFAULT_NOTE_DURATION
    _add_note_to_sequence(
...
def flush_note_decoding_state(
  ...
  for onset_time, _ in state.active_pitches.values():
    state.current_time = max(state.current_time, onset_time + Consts.MIN_NOTE_DURATION)
  for (pitch, program) in list(state.active_pitches.keys()):
    onset_time, onset_velocity = state.active_pitches.pop((pitch, program))
    _add_note_to_sequence(
...
class NoteEncodingSpecType(runLengthEncoding.EventEncodingSpec):
  pass
