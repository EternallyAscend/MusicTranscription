...
def num_velocity_bins_from_codec(codec: eventCodec.Codec):
...
def drop_programs(tokens, codec: eventCodec.Codec):
...
class ProgramGranularity:
  ...
  tokens_map_fn: Callable[[Sequence[int], eventCodec.Codec], Sequence[int]]
...
def build_codec(vocab_config: VocabularyConfig):
  """Build event codec."""
  event_ranges = [
      eventCodec.EventRange('pitch', note_seq.MIN_MIDI_PITCH,
                             note_seq.MAX_MIDI_PITCH),
      # velocity bin 0 is used for note-off
      eventCodec.EventRange('velocity', 0, vocab_config.num_velocity_bins),
      ...
      eventCodec.EventRange('tie', 0, 0),
      eventCodec.EventRange('program', note_seq.MIN_MIDI_PROGRAM,
                             note_seq.MAX_MIDI_PROGRAM),
      eventCodec.EventRange('drum', note_seq.MIN_MIDI_PITCH,
                             note_seq.MAX_MIDI_PITCH),
  ]

  return eventCodec.Codec(
...
def vocabulary_from_codec(codec: eventCodec.Codec) -> seqio.Vocabulary:
  return GenericTokenVocabulary(
      codec.num_classes, extra_ids=T5_DATA_DEFAULT_EXTRA_IDS)
...
class GenericTokenVocabulary(seqio.Vocabulary):
  ...
  def _encode_numpy(self, token_ids):
    ...
    with np.errstate(invalid='raise'):
      ...
      assert np.all(np_ids >= 0), f"Negative values are not allowed: {np_ids}"
      assert np.all(np_ids < self._num_regular_tokens), f"Out of bounds token id: {np_ids}"
      return np_ids + self._num_special_tokens
...
#     return tf_ids
  ...
  def _decode_numpy(self, ids):
    ...
    eos_and_after = np.cumsum(np.where(ids == self.eos_id, 1, 0)) >= 1
    decoded_ids = np.where(
            eos_and_after,
            DECODED_EOS_ID,
            np.where(
                np.logical_and(
                    np.greater_equal(ids, self._num_special_tokens),
                    np.less(ids, self._base_vocab_size)),
